...
import com.example.householdAccount.requestDto.user.SetIncomeItemReqDto.IncomeItemList;
import com.example.householdAccount.responseDto.user.CommonResDto;
import com.example.householdAccount.util.CommonUtil;

@Service
public class SetIncomeItemService {
	...
	public CommonResDto setIncomeItem(SetIncomeItemReqDto reqDto) {
		//レスポンスオブジェクト定義
		...
		try {
			//現状のID最大値を取得
			String maxId = selectItemImpl.getMaxIdIncomeItem();
			Integer incomeId = commonUtil.getStartId(maxId);
			for(IncomeItemList incomeItem : reqDto.getIncomeItemList()) {
				registItemImpl.setIncomeItem(reqDto.getUserId(), incomeId.toString(), incomeItem.getIncomeItemName());
				incomeId ++;
			}
			setSuccessReqInfo();
		}catch(Exception e) {
			setErrorReqInfo(HouseholdAccountConstant.NOT_REGIST_INCOME_ITEM_ERROR);
			return resDto;
		}
	 ...
	 */
	private boolean validationCheck(SetIncomeItemReqDto reqDto) {
		boolean flg = false;
		...
		if(reqDto.getIncomeItemList() == null && reqDto.getIncomeItemList().isEmpty()) {

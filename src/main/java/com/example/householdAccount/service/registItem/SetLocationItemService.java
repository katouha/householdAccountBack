...
import com.example.householdAccount.requestDto.user.SetLocationItemReqDto.LocationItemList;
import com.example.householdAccount.responseDto.user.CommonResDto;
import com.example.householdAccount.util.CommonUtil;

@Service
public class SetLocationItemService {
	...
	public CommonResDto setLocationItem(SetLocationItemReqDto reqDto) {
		//レスポンスオブジェクト定義
		...
		try {
			//現状のID最大値を取得
			String maxId = selectItemImpl.getMaxIdLocationItem();
			Integer locationId = commonUtil.getStartId(maxId);
			for(LocationItemList locationItem : reqDto.getLocationItemList()) {
				registItemImpl.setLocationItem(reqDto.getUserId(), locationId.toString(), locationItem.getLocationItemName());
				locationId ++;
			}
			setSuccessReqInfo();
		}catch(Exception e) {
			setErrorReqInfo(HouseholdAccountConstant.NOT_REGIST_LOCATION_ITEM_ERROR);
			return resDto;
		}
	 ...
	 */
	private boolean validationCheck(SetLocationItemReqDto reqDto) {
		boolean flg = false;
		...
		if(reqDto.getLocationItemList() == null && reqDto.getLocationItemList().isEmpty()) {

...
public class UpdateUserPasswordService {
	...
	public CommonResDto updateUserPassword(UpdateUserPasswordReqDto reqDto) {
		//レスポンスオブジェクト定義
		if(validationCheck(reqDto)) {
			return resDto;
		}
		try {
			userImpl.updateUserPassword(reqDto.getUserId(), reqDto.getPassword());
		}catch(Exception e) {
			setErrorReqInfo(HouseholdAccountConstant.UPDATE_USER_PASSWORD_ERROR);
			return resDto;
		}
	 ...
	 */
	private boolean validationCheck(UpdateUserPasswordReqDto reqDto) {
		boolean flg = false;
	 ...
	 * @param req(リクエスト内容)
	 */
	private void setSuccessReqInfo(UpdateUserPasswordReqDto req) {
